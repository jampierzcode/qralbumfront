...
import { REQUEST, REVIEW, STATUS } from "../lib/gifts.js";
...
/** Pedido hecho por un cliente final desde la tienda pública. */
export function RequestBadge({ requestStatus }) {
  if (!requestStatus || requestStatus === "none") return null;
  const meta = REQUEST[requestStatus];
  return <span className={`adm-status adm-status--${meta.tone}`}>{meta.label}</span>;
}
